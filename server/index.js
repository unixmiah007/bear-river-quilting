...
app.post('/api/admin/init-db', authMiddleware, async (_req, res) => {
  ...
  try {
    ...
    // Apply incremental schema repairs
    await ensureProductInventoryColumns(conn);
    log.push('product inventory columns ensured');
    ...
    // Seed demo products only when the table is empty
    const [[{ cnt }]] = await conn.query('SELECT COUNT(*) AS cnt FROM products');
    let seeded = false;
    if (Number(cnt) === 0) {
      await seedDemoProducts(conn);
      seeded = true;
      log.push('demo products seeded');
    } else {
      log.push(`products table already has ${cnt} row(s) — skipped seeding`);
    }

    res.json({ ok: true, seeded, log });
  } catch (e) {
    console.error('[init-db]', e);
    res.status(500).json({ ok: false, error: e.message || 'Database initialisation failed', log });
  } finally {
    conn.release();
  }
});
